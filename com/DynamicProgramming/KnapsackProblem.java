...
public class KnapsackProblem {
     ...
     * recursion tree, K(1, 1) is being evaluated twice. Time complexity of this naive recursive solution is
     * exponential (2^n).
     *
     ...
     * Since suproblems are evaluated again, this problem has Overlapping Subprolems property. So the 0-1 Knapsack
     * problem has both properties (see this and this) of a dynamic programming problem. Like other typical Dynamic
     * Programming(DP) problems, recomputations of same subproblems can be avoided by constructing a temporary
     * array K[][] in bottom up manner. Following is Dynamic Programming based implementation.
     */
        ...
        for (i = 0; i <= n; i++)
        {
            for (w = 0; w <= W; w++)
            {
                if (i==0 || w==0)
                    ...
                    K[i][w] = max2(val[i-1] + K[i-1][w-wt[i-1]],  K[i-1][w]);
                else
                    K[i][w] = K[i-1][w];
            }
        }
