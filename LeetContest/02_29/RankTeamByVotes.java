...
public class RankTeamByVotes {
    ...
    public static void main(String[] args) {
        String []A =  {"FVSHJIEMNGYPTQOURLWCZKAX","AITFQORCEHPVJMXGKSLNZWUY","OTERVXFZUMHNIYSCQAWGPKJL","VMSERIJYLZNWCPQTOKFUHAXG",
...
//"VWFHSJARNPEMOXLTUKICZGYQ"
    ...
    public static String rankTeams(String[] votes) {
        ...
        List<String> vt = Arrays.asList(votes);
        for(String x: vt) {
            //   vt.stream().forEach(x->{

            for (int i = 0; i < x.length(); i++) {
                if (ranks.get(String.valueOf(x.charAt(i))) != null) {
                    RANK rnk = ranks.get(String.valueOf(x.charAt(i)));
                    rnk.weight = rnk.weight + i;
                } else {
                    RANK rnk = new RANK();
                    rnk.name = String.valueOf(x.charAt(i));
                    rnk.weight = i;
                    ranks.put(String.valueOf(x.charAt(i)), rnk);
                }
            }
        }
   //     });
       ...
       ranks.entrySet().stream().forEach(x->{
           list.add( x.getValue());
       });
        ...
        Collections.sort(list,(x,y) ->{
            ...
            if(x.weight < y.weight){
                ...
                return x.name.charAt(0) - y.name.charAt(0);
